...
const sideBar = menuOpen
  ...
  <Link href={"/user/cart"} className='relative bg-white rounded-full w-11 h-11 flex items-center justify-center shadow-md hover:scale-105 transition'>
    ...
    <span className='absolute -top-1 -right-1 bg-red-500 text-white text-xs w-5 h-5 flex items-center justify-center rounded-full font-semibold shadow-md'>
      {cartData.length}
    </span>
        </Link>
        </>}
          ...
          <AnimatePresence>
            {open && 
              ...
              >
                ...
                {user.role=="user" &&  <Link href={"/user/my-orders"} className='flex items-center gap-2 px-3 hover:bg-green-50 rounded-lg text-gray-700 font-medium' onClick={()=>setOpen(false)}>
